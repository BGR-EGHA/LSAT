...
cpdef slope_degree(np.ndarray[DTYPE_t, ndim=2]InArray, float nodata, float cellsize, char * type):
    ...
    if type == "DEGREE":
        slope = np.arctan(np.sqrt((dz_dx)**2 + (dz_dy)**2)) * 180 / 3.14159265
    else:
        slope = np.arctan(np.sqrt((dz_dx)**2 + (dz_dy)**2)) * 100
